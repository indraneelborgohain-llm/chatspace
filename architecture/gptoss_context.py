...
class ModelConfig:
    ...
    # Context dimension is same as hidden_size (output of each transformer block)
    @property
...
class MLPBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        # New: Context projection for MLP
        self.context_proj = torch.nn.Linear(
            config.context_dim, config.hidden_size, device=device, dtype=torch.bfloat16
        )
...
class Transformer(torch.nn.Module):
    def __init__(
        ...
        # Context state: stores the output from the previous prediction
        # Initialized to zeros, shape (hidden_size,)
    ...
    def reset_context(self):
        """Reset context state to zeros (call at start of new conversation)"""
        self.context_state.zero_()

    def forward(self, x: torch.Tensor, update_context: bool = True) -> torch.Tensor:
        """
        Args:
            x: Token indices, shape (seq_len,)
            update_context: If True, updates context_state with final output (default: True)
                           Set to False during evaluation/inference if you don't want to update
        
        Returns:
            Logits of shape (seq_len, vocab_size)
        """
        # Get token embeddings: (seq_len, hidden_size)
        token_embeds = self.embedding(x)
        
        # Prepend context state as first token: (1, hidden_size)
        context_embed = self.context_state.unsqueeze(0)
        
        # Concatenate: (seq_len + 1, hidden_size)
        x = torch.cat([context_embed, token_embeds], dim=0)
        
        # Context vector is just the first position (the prepended context)
        # We'll broadcast it to all positions
        ...
        # Remove the first position (context position) to get back to original seq_len
        x_tokens = x[1:]
        
        # Get logits for the actual tokens
        logits = self.unembedding(x_tokens)
        
        # Update context state with the last token's output (before unembedding)
...
class TokenGenerator:
    ...
    def __init__(self, checkpoint: str, device: torch.device):
        ...
        # Reset context at initialization
    ...
    def generate(self,
        ...
        Args:
            ...
            return_logprobs: Whether to yield log probabilities
            reset_context: If True, reset context before generation (start new conversation)
        """
        if reset_context:
            self.model.reset_context()
        ...
        while max_tokens == 0 or num_generated_tokens < max_tokens:
            # Forward pass - context automatically updates
            logits = self.model(
